...
public class AVLTree {
    ...
    public AVLNode insert(AVLNode node, String[] data) {
        ...
        if (node.data[0].compareTo(data[0]) > 0) {
        ...
        // Rotacion left
        if (balDiff > 1 && node.left.data[0].compareTo(data[0]) < 0 ) {
            return rightRotate(node);
        }

        // Rotacion Right
        if (balDiff < -1 && node.right.data[0].compareTo(data[0]) < 0) {
    ...
    public String search(AVLNode root, String key)
    {
        if (root==null || root.data[0] == key)
            return root.data[1];
